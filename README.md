...
安装程序基于自定义 Flutter 安装器界面与 7-Zip SFX 打包,带 GUI 向导、圆角无边框窗口、应用图标、开始菜单/桌面快捷方式。
它会检测本机已安装的任意版本,默认路径指向旧安装目录;若安装包版本高于已装版本会显示为更新,
低于已装版本会显示为回退,相同版本显示为重新安装。
本地构建需安装 7-Zip,并确保 `7z.exe` 与 `7z.sfx` 可在安装目录中找到。
...
`build\app\outputs\flutter-apk\liuyao-v1.0.2-3-android-release.apk`。
