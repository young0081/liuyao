...
安装包为 zip 结构,解压后入口程序是 `liuyao_installer.exe`;打开后第一屏即为 Flutter GUI 安装界面。
...
产物位于 `dist\liuyao-installer-windows-x64-v<version>.zip`。
...
`build\app\outputs\flutter-apk\liuyao-v1.0.3-4-android-release.apk`。
